...
var game_instance_name = "falling_block_game"
...
func _ready():
	...
	label_high_score_value.set_pos(Vector2(336+label_high_score_text.get_pos().x, label_high_score_text.get_pos().y))
	label_current_score_value.set_pos(Vector2(336+label_high_score_text.get_pos().x, label_current_score_text.get_pos().y))
...
func create_game():
	...
	var game_instance = load("res://scenes/games/falling_block_game/falling_block_game.tscn").instance()
...
#	#game_instance.set_pos(Vector2(0,32))
	...
	game_instance.colorset[0] = settings.get_colour_left_eye()
	game_instance.colour_idle_block = settings.get_colour_right_eye()
	
	#game_instance.snake_tail_colour = settings.get_colour_right_eye()
	game_instance.set_pos(Vector2((5*game_instance.unit_size),game_instance.unit_size/2+(screen_size.y-game_instance.board_height*game_instance.unit_size)/2))
	canvas.set_size(Vector2(game_instance.board_width*game_instance.unit_size, game_instance.board_height*game_instance.unit_size))
	canvas.set_pos(Vector2(game_instance.get_pos().x-game_instance.unit_size/2,game_instance.get_pos().y-game_instance.unit_size/2))
	add_child(game_instance)
	game_instance.connect("game_over", self, "_on_game_instance_game_over", [], CONNECT_ONESHOT)
	game_instance.connect("lines_cleared", self, "_on_game_instance_lines_cleared")
	timer.start()
...
func set_current_score(value):
	...
	if value > settings.get_high_score_falling_block():
		settings.set_high_score_falling_block(value)
		label_high_score_value.set_text(str(settings.get_high_score_falling_block()))
	current_score = value
...
func _on_button_play_again_pressed():
	get_node(game_instance_name).queue_free()
	create_game()
	pass
...
func _on_game_instance_lines_cleared(lines_cleared):
	#print(lines_cleared)
